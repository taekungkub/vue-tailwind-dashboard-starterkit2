    ...
    <div class="grid grid-cols-1 gap-4">
      <div class="justify-center items-center">
        <div class="card-chart">
          <div id="candle-chart" ref="ChartRef"></div>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  mounted() {
    this.elem = document.getElementById("candle-chart");

    var chart = LightWeightCharts.createChart(this.elem, {
      width: 600,
      height: 300,
      layout: {
    ...
    this.chartCreate = chart;

    // new ResizeObserver((entries) => {
    //   if (entries.length === 0 || entries[0].target !== this.elem) {
    //     return;
    //   }
    //   const newRect = entries[0].contentRect;
    //   this.chartCreate.applyOptions({ height: newRect.height, width: newRect.width });
    // }).observe(this.elem);
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      this.chartCreate.applyOptions({ height: cr.height, width: cr.width });
    });

    ro.observe(this.$refs.ChartRef);
  },
};
</script>
